...
```mermaid
flowchart TD
    Tauri[Tauri native executable] --> React[React UI]
    Browser[Browser fallback / development shell] --> React
    React <--> WS[localhost WebSocket protocol-v1]
    WS <--> Companion[packaged Python companion]
    Companion --> Supervisor[supervisor / Sam core]
    Supervisor --> Models[local model adapters]
    Supervisor --> Voice[STT / TTS adapters]
    Supervisor --> Policy[policy / approved tools]
```

A release build resolves only the fixed `companion/sam-supervisor.exe` beneath
Tauri's resource directory. A reproducible cx_Freeze directory build packages
CPython, Sam, runtime dependencies, static resources, configuration example,
VC runtime files, and notices; its manifest records version, file inventory,
and content hash. Tauri passes a per-user application-data root, never the
installed program directory, and owns only that direct companion child. The
frontend cannot choose an executable or gain shell permission. Models, Whisper
weights, LM Studio, and Ollama remain external.

The Windows NSIS build maps the whole companion directory into Tauri resources
and uses a per-user install. Rust/Cargo/MSVC/Windows SDK/cx_Freeze are build-time
requirements only; installed Sam needs Windows WebView2 plus separately chosen
model and speech prerequisites. Browser mode continues to run the same React UI
and protocol from the Python package. Linux companion/package validation and
supervisor self-update remain separate work. The current unsigned cx_Freeze
launcher triggered Norton's `IDP.Generic` heuristic. Packaging therefore emits
development-only output until antivirus review and trusted Authenticode signing
are part of the release process; this is a distribution gate, not a policy bypass.
cx_Freeze remains the selected strategy because its permissive PSF licensing,
directory layout, native-extension handling, and isolated companion entry points
fit the current runtime. A later release decision may compare other permissive
approaches using measured AV reputation, reproducibility, size, updateability,
native-dependency handling, and CI reliability; this observation alone does not
justify an unvalidated packager switch.
